...
TOOLBELT_PROJECT_ROOT="$(_bootstrap_abspath "$(dirname "$_PW_BOOTSTRAP_PATH")")"
export TOOLBELT_PROJECT_ROOT
PW_PROJECT_ROOT="$TOOLBELT_PROJECT_ROOT"
PW_ROOT="$TOOLBELT_PROJECT_ROOT/third_party/pigweed"

# Set your project's banner and color.
PW_BRANDING_BANNER="$TOOLBELT_PROJECT_ROOT/banner.txt"
...
if [ ! -e "$PW_ROOT/pw_env_setup/util.sh" ]; then
  ...
  git -C "$MARKSMEN_PROJECT_ROOT" submodule update --init
fi
